...
func _physics_process(_delta):
	target.x = clamp(target.x, 1, Global.VP.x - 2*width)
	position = target
...
func hit(_ball):
	...
	$Images/Paddle.position.y = 30
	tween.tween_property($Images/Paddle, "position:y", 10, time_bounce).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
